...
import org.dpppt.backend.sdk.model.BeaconRequest;
...
public class DPPPTController {
	...
	public DPPPTController(DPPPTDataService dataService, String appSource,
		...
		ValidationUtils validationUtils, long releaseBucketDuration,
		long requestTime, int retentionDays, int codeValidDays)
	{
	...
	public String determinesAppSource(String userAgent)
	{
		String appSource;

		if (userAgent.contains("Mobi"))
		{
			appSource = "Applacovid";
		}
		else
		{
			appSource = "AdminConsole";
		}

		return appSource;
	}

	public boolean isValidCovidCode(String value)
	{
		return dataService.isValidCovidCode(value, codeValidDays);
	}
			...
			@AuthenticationPrincipal Object principal) throws InvalidDateException
	{
		...
		/*if(!isValidCovidCode(exposeeRequest.getAuthData().getValue()))
		{
			return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
		}*/
		...
		String appSource = determinesAppSource(userAgent);
	...
	@Documentation(
		description = "Send beacon information to server",
		responses = {
			"200=>The beacon information has been stored in the database",
			"403=>Authentication failed"
		})
	public @ResponseBody ResponseEntity<String> addBeaon(@Valid @RequestBody
	@Documentation(description = "The BeaconRequest contains the application uuid to indicate whose is registering a route, the beacon uuid, major, and minor to identify the beacon, and the rssi which is the intensity of the beacon.")
		...
		@AuthenticationPrincipal Object principal) throws InvalidDateException
	{
		...
		BeaconRoute route = new BeaconRoute();
		route.setAppUuid(beaconRequest.getAppUuid());
		route.setBeaconUuid(beaconRequest.getBeaconUuid());
		route.setMajor(beaconRequest.getMajor());
		route.setMinor(beaconRequest.getMinor());
		route.setTimestamp(beaconRequest.getTimestamp());
		route.setRssi(beaconRequest.getRssi());

		dataService.addBeacon(route);
		return ResponseEntity.ok().build();
	}
	...
	public @ResponseBody ResponseEntity<ExposedOverview> getExposedByDayDate(@PathVariable
		...
		List<Exposee> exposeeList = dataService.getSortedExposedForBatchReleaseTime(batchReleaseTime, releaseBucketDuration, retentionDays);
	...
	public @ResponseBody ResponseEntity<Exposed.ProtoExposedList> getExposedByBatch(@PathVariable
		...
		logger.debug("protoExposee..\n");
		for (org.dpppt.backend.sdk.model.proto.Exposed.ProtoExposee e : protoExposee.getExposedList())
		{
			logger.debug(" - " + e.getKey() + " - " + e.getKeyDate());
		}
