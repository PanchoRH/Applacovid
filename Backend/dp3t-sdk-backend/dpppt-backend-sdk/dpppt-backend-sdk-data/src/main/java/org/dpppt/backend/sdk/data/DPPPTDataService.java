...
public interface DPPPTDataService {
	 ...
	 */
	List<Exposee> getSortedExposedForBatchReleaseTime(long batchReleaseTime, long releaseBucketDuration, int retentionDays);
	...
	/**
	 * Upserts (Update or Inserts) the given beaconPath
	 *
	 * @param beaconPath   the beacon path of the mobile app.
	 */
	void addBeacon(BeaconRoute beaconPath);

	/**
	 * Verifies if the covid code given is valid and not higher of code valid days.
	 * @param code The covid code.
	 * @param codeValidDays How many days the code will be valid.
	 * @return True if the covid code is valid, otherwise false.
	 */
	boolean isValidCovidCode(String code, int codeValidDays);
}
