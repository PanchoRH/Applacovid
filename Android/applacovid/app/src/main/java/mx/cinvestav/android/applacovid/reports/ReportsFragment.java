 ...
 */
package mx.cinvestav.android.applacovid.reports;
...
import mx.cinvestav.android.applacovid.util.DateUtils;
import mx.cinvestav.android.applacovid.util.NotificationUtil;
import mx.cinvestav.android.applacovid.util.PhoneUtil;
...
public class ReportsFragment extends Fragment {
	...
	public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		callHotlineButton1.setOnClickListener(view1 -> openLink(R.string.meldungen_explanation_link_url)); // (gusorh)
		callHotlineButton2.setOnClickListener(view1 -> openLink(R.string.meldungen_explanation_link_url)); // (gusorh)
			...
			if (tracingStatusInterface.isReportedAsInfected()) {
				...
				for (int i = 0; i < exposureDays.size(); i++) {
					...
					if (i == 0) {
						...
						int daysDiff = DateUtils.getDaysDiffUntil(exposureTimestamp, 15);
						if (daysDiff == 1) {
	...
	public void onResume() {
		...
		if (lastHotlineCallTimestamp != 0) {
			...
			//callHotlineLastText1.setText(date);  // (gusorh)
			//callHotlineLastText2.setText(date);  // (gusorh)
