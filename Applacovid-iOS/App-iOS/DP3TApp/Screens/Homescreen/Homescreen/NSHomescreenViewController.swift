...
class NSHomescreenViewController: NSTitleViewScrollViewController {
    ...
    private let whatToDoPositiveTestButton = NSWhatToDoView()
    private let codeQRTestView = NSQRTestView()
    ...
    /*private let whatToDoPositiveTestButton2 = NSWhatToDoButton(title: "Generar QR para prueba covid", subtitle: "", image: UIImage(named: "qr-icon"))*/

    private let debugScreenButton = NSButton(title: "debug_settings_title".ub_localized, style: .outlineUppercase(.ns_purple))
    ...
    override func viewDidLoad() {
        ...
        codeQRTestView.touchUpCallback = { [weak self] in
                   guard let strongSelf = self else { return }
                   strongSelf.showQRTest()
               }
    ...
    private func setupLayout() {
        ...
        stackScrollView.addArrangedView(codeQRTestView)
        stackScrollView.addSpacerView(NSPadding.large)
    ...
    func updateState(_ state: UIStateModel) {
        ...
        codeQRTestView.isHidden = isInfected
    ...
    private func showQRTest() {
        navigationController?.pushViewController(NSQRTestViewController(), animated: true)
    }

    private let uploadDBButton = NSButton(title: "Upload DB to server", style: .outlineUppercase(.ns_purple))
