...
class NSOnboardingPermissionsViewController: NSOnboardingContentViewController {
    ...
    let permissionButton = NSButton(title: "", style: .normal(.ns_purple))

    private let goodToKnowContainer = UIView()
    private let goodToKnowLabel = NSLabel(.textLight, textColor: .ns_purple)
    ...
    private func fillViews() {
        ...
        case .push:
            ...
            let info1 = NSOnboardingInfoView(icon: UIImage(named: "ic-tracing-onboarding")!, text: "Applacovid actualiza periódicamente los reportes de contagios recibidos", title: "onboarding_push_gtk_title1".ub_localized)
            let info2 = NSOnboardingInfoView(icon: UIImage(named: "ic-meldung")!, text: "onboarding_push_gtk_text1".ub_localized, title: nil)
            elements.append(info1)
            elements.append(info2)
